...
def build_seed(user: Dict[str, Any], profile: Optional[Dict[str, str]] = None) -> str:
    if profile:
        ...
        if cpf:
            return cpf
    return (
        validation.coerce_text(user.get("email"))
        or validation.coerce_text(user.get("sub"))
        or "toyotatech"
    )
...
def select_vehicle(seed: str) -> Dict[str, str]:
    digest = hashlib.sha1(
        (validation.coerce_text(seed) or "toyotatech").encode("utf-8")
    ).hexdigest()
...
def build_financing(seed: str) -> Dict[str, Any]:
    ...
    total_installments = installment_options[
        int(digest[:2], 16) % len(installment_options)
    ]
...
def build_documents(seed: str, purchase_date: str, vehicle: Dict[str, str]) -> list:
    ...
    return [
        ...
        },
        {
            ...
            "title": "CRLV-e",
            "date": purchase_date,
            "status": "available" if int(digest[16:18], 16) % 3 else "pending",
        },
        {
            "id": "warranty",
        ...
        {
            "id": "manual",
            "title": f"Manual do {vehicle_model}",
            "date": purchase_date,
            "status": "available",
        },
        {
...
def build_garage(
    user: Dict[str, Any], profile: Optional[Dict[str, str]] = None
) -> Dict[str, Any]:
