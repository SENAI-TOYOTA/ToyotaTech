...
def _query_index(table: Any, index_name: str, key_name: str, key_value: str, *, limit: int = 25) -> List[Dict[str, Any]]:
    try:
        result = table.query(
...
def find_by_purchase_id(purchase_id: str) -> Optional[Dict[str, Any]]:
    try:
        result = get_table(PURCHASE_TABLE_NAME).get_item(Key={"purchaseId": purchase_id})
...
def find_by_order_id(order_id: str) -> Optional[Dict[str, Any]]:
    return _query_single_item(get_table(PURCHASE_TABLE_NAME), "orderId-index", "orderId", order_id)
...
def find_all_by_cpf(cpf: str) -> List[Dict[str, Any]]:
    ...
    if not normalized_cpf:
        return []
    return _query_index(get_table(PURCHASE_TABLE_NAME), "cpf-index", "cpf", normalized_cpf)


def find_linked_for_user(user_id: str) -> Optional[Dict[str, Any]]:
    return _query_single_item(get_table(PURCHASE_TABLE_NAME), "userId-index", "userId", user_id)
...
def matches_profile(purchase: Dict[str, Any], profile: Dict[str, str]) -> bool:
    ...
    purchase_cpf = validation.normalize_cpf(purchase.get("cpf") or customer_value(purchase, "cpf"))
...
def attach_to_user(purchase: Dict[str, Any], user_id: str) -> Optional[Dict[str, Any]]:
    ...
    get_table(PURCHASE_TABLE_NAME).update_item(
        ...
        ExpressionAttributeValues={":userId": user_id, ":linkedAt": now, ":updatedAt": now},
    )
...
def seed_for_profile(user: Dict[str, Any], profile: Dict[str, str]) -> Optional[Dict[str, Any]]:
    table = get_table(PURCHASE_TABLE_NAME)
    purchase = demo.build_purchase(user, profile)
    try:
        table.put_item(Item=purchase, ConditionExpression="attribute_not_exists(purchaseId)")
        return purchase
    except ClientError as error:
        code, _ = error_body(error)
        if code == "ConditionalCheckFailedException":
            existing = find_by_purchase_id(validation.coerce_text(purchase.get("purchaseId")))
            if is_generated(existing):
                table.put_item(Item=purchase)
                return purchase
            return existing
        if code in {"ResourceNotFoundException", "AccessDeniedException", "ValidationException"}:
            return None
        raise
